...
**Remark: (f)** Intuitively, if the sequence converged, the limit would be $$. However, $1$ appears indefinitely (though less and less frequently), the limit cannot be $$. For people who are interested in the rigorous proof using the definition of limit, here is the argument. For sake of contradiction, suppose the limit of the sequence is $L$. Let $\delta = 1/2$. There should be $N$ so that $L - 1/2 < a_n < L + 1/2$ for all $n > N$. However, this means $L -1/2 < 0$ and $1 < L+1/2$, hence $1/2 < L < 1/2$, which yields a contradiction.
